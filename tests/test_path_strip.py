...
def test_paint_path_row_applies_trim_path_trim():
    ...
    assert row[2] not in ("GRASS", TRIM_BLOCK)
    assert row[3] not in ("GRASS", TRIM_BLOCK)
    assert row[4] not in ("GRASS", TRIM_BLOCK)
    assert row[5] == TRIM_BLOCK
    assert row[6] == "GRASS"


def test_paint_path_at_site_row():
    ground = [["GRASS"] * 5 for _ in range(3)]
...
def test_paint_path_at_site_vertical_orientation():
    ...
    assert ground[2][2] not in ("GRASS",)
    assert ground[4][0] == "GRASS"


def test_paint_path_skips_structure_footprint():
    ground = [["GRASS"] * 9 for _ in range(8)]
    paint_path_at_site(
        ground,
        site_x=7,
        site_z=2,
    ...
    for z in range(1, 4):
        for x in range(2, 6):
            assert ground[z][x] == "GRASS"

    assert ground[0][7] == TRIM_BLOCK
    assert ground[2][7] != "GRASS"
...
def test_paint_path_rejects_anchor_on_structure():
    ...
    assert all(cell == "GRASS" for row in ground for cell in row)


def test_random_path_block_only_dirt_path_when_no_variety():
    assert random_path_block(variety_blocks=[]) == "DIRT_PATH"


def test_random_path_block_respects_variety_subset():
    rng = random.Random(0)
    tokens = {random_path_block(rng, variety_blocks=["DIRT"]) for _ in range(50)}
    assert "DIRT" in tokens
    assert tokens <= {"DIRT_PATH", "DIRT"}


def test_resolve_trim_and_variety_defaults():
    assert resolve_trim_block({}) == "GRAVEL"
...
def test_repaint_existing_path_respects_orientation_axis():
    ...
    for z in range(8):
        if ground[z][3] in ("GRAVEL", "COBBLESTONE"):
            assert ground[z][3] == "COBBLESTONE"
    ...
    assert ground[4][1] == "COBBLESTONE"
    assert ground[4][5] == "COBBLESTONE"


def test_erase_path_at_site_clears_entire_row():
    ground = [["GRASS"] * 12 for _ in range(3)]
    paint_path_row(ground[0], center_x=2, path_width=3, site_z=0, rng=random.Random(0))
