...
def test_history_roundtrip_preserves_group_rename_and_filter():
    document = StructureDocument(
        structure_path=__file__,
        metadata={
            "grid": {
        ...
        layer_files=["layer_00.yaml"],
        layer_paths=[__file__],
        layers=[{"group": "Basement", "index": -1, "cells": [["STONE"]]}],
        site_ground=[["GRASS"] * 10 for _ in range(10)],
    )
    ...
    snapshot = capture_history_state(
        ...
        dirty_layers=set(),
        dirty_structure=False,
        group_filter="Basement",
    )

    document.layers[0]["group"] = "Cellar"
    document.metadata["grid"]["groups"] = ["Cellar"]

    dirty_flag = [True]
    group_filter_flag: list[str | None] = ["Cellar"]
    apply_history_state(
        document,
        snapshot,
        dirty_layers=set(),
    ...
    assert document.layers[0]["group"] == "Basement"
    assert document.metadata["grid"]["groups"] == ["Basement"]
    assert group_filter_flag[0] == "Basement"
