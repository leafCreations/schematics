...
| `card-done-forward-feedback-skipped` | Card Done lessons without `## Forward-looking feedback` | Run kanban-markdown § Card Done step 6 after Lessons captured; six categories on done card — not chat §6 | kanban-markdown | kanban-review-qa.mdc#card-done-forward-feedback |
| `card-done-forward-feedback-top3-skipped` | Card Done without `### Top forward feedback` in chat | After card block, surface top 3 items before `### Files used` — agent-self-evaluation §7 | agent-self-evaluation | agent-self-evaluation.mdc |
