...
| `agent-costly-discovery-commands` | `find`, broad `Glob` on `done/`/`archived/`, recovery search, broad explore after classify | Track in command ledger; ff honest risk **≥ 3**; optional `### Commands used` on Card Done — Signature: `agent-costly-discovery-commands` | agent-self-evaluation | kanban-agent-cards.mdc |
| `kanban-card-move-resolver` | `mv` failed on Card Done; agent ran `find` on `.devtool/features` | `python3 scripts/move_kanban_card.py --id {id} --to done`; no parallel edit+mv — Signature: `kanban-card-move-resolver` | kanban-markdown | kanban-review-qa.mdc |
